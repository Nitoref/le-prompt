...
segmentGit2(PromptOpt *p)
{
       ...
       | stats.conflicted )
    {
        ...
        if(!strcmp(p->args->GitMode, "compact"))
        {
            ...
            if (stats.ahead)
            {
                strcat(statsStr, p->symbols->GitAhead);
            }
            if (stats.behind)
            {
                strcat(statsStr, p->symbols->GitBehind);
            }
            if (stats.staged)
            {
                strcat(statsStr, p->symbols->GitStaged);
            }
            if (stats.notStaged)
            {
                strcat(statsStr, p->symbols->GitNotStaged);
            }
            if (stats.untracked)
            {
                strcat(statsStr, p->symbols->GitUntracked);
            }
            if (stats.conflicted)
            {
                strcat(statsStr, p->symbols->GitConflicted);
            }
